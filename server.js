...
// Updated auth status endpoint for multi-user system
app.get('/api/auth/status', async (req, res) => {
  try {
    ...
    console.log('Auth status check for user:', sessionUser.email);
    
    // Get user from database to check tokens
    const userResult = await pool.query('SELECT * FROM users WHERE id = $1', [sessionUser.id]);
    ...
    if (isAuthenticated) {
      console.log('Testing tokens validity for user:', user.email);
      try {
        // Create user-specific OAuth client and test tokens
        const userOAuthClient = createUserOAuth2Client(user);
        if (userOAuthClient) {
          const calendar = google.calendar({ version: 'v3', auth: userOAuthClient });
          await calendar.calendarList.list({ maxResults: 1 });
          console.log('Tokens are valid for user:', user.email);
        }
      } catch (tokenError) {
        console.log('Token validation failed for user:', user.email, tokenError.message);
        // Don't invalidate immediately - let calendar operations handle token refresh
      }
    }
    ...
    res.json({
      ...
      accessToken: isAuthenticated ? user.access_token : null,
      user: {
        id: sessionUser.id,
      ...
      message: isAuthenticated ? 'Google Calendar conectado para ' + user.email : 'Google Calendar desconectado'
    });
  } catch (error) {
...
app.get('/login', (req, res) => {
  ...
  res.send(String.raw`<!DOCTYPE html>
    ...
    <head>
      ...
      <title>Login - IntoTheCom CRM</title>
      <style>
        * {
          margin: 0;
          padding: 0;
    ...
    <body>
      <div class="login-container">
        <div class="logo">📊 IntoTheCom</div>
        <div class="subtitle">CRM & Management System</div>
        
        <div class="welcome-text">
          Bienvenido al CRM de IntoTheCom. Ingresa con tu cuenta de Google para acceder a todas las funcionalidades.
        </div>
        ...
        <div class="restriction-note">
          🔒 Solo usuarios con email <strong>@intothecom.com</strong> pueden acceder al CRM.
        </div>
        ...
        async function loginWithGoogle() {
          try {
            ...
            if (data.success && data.authUrl) {
              ...
              alert('Error: ' + (data.error || 'Authentication not configured'));
      ...
      <style>
        ...
        * {
        ...
        .form-group textarea:focus {
          ...
          box-shadow: 0 0 0 3px rgba(99, 102, 241, 0.2), 0 8px 32px rgba(0, 0, 0, 0.1);
          background: rgba(255, 255, 255, 0.15);
          transform: translateY(-2px);
        }
        ...
        .chat-input-area:focus-within {
          border-color: var(--primary-color);
          box-shadow: 0 0 0 3px rgba(99, 102, 241, 0.2), 0 8px 32px rgba(0, 0, 0, 0.15);
          background: rgba(255, 255, 255, 0.15);
          transform: translateY(-2px);
        }
...
// Get user profile (multi-user version)
...
// Save user profile (multi-user version)
app.post('/api/profile', requireAuth, async (req, res) => {
  try {
    const userEmail = req.user.email;
    console.log('Saving profile for user:', userEmail);
...
// Upload profile photo (multi-user version)
